...
public class DungeonBuilder : SingletonMonobehavior<DungeonBuilder>
{
    ...
    /// <summary>
    /// Attempt to randomly build the dungeon for the specified room node graph. returns true if a successful random layout was genrated, 
    ...
    private bool ProcessRoomsInOpenRoomNodeQueue(RoomNodeGraphSO roomNodeGraph, Queue<RoomNodeSO> openRoomNodeQueue, bool noRoomOverlaps)
    {
        ...
        while (openRoomNodeQueue.Count > 0 && noRoomOverlaps == true)
        {
            ...
            // if the room is the entrance mar as positioned and add to room dictionary
    ...
    private bool PlaceTheRoom(Room parentRoom, Doorway doorwayParent, Room room)
    {
        ...
        // Calculate adjustment position offset based on room doorway position that we are trying to connect (e.g. if this doorway is west then we need to 
        // add (1,0) to the east parent doorway)
    ...
    /// <returns></returns>
    private bool IsOverLappingRoom(Room roomOne, Room roomTwo)
    {
        bool isOverlappingX = IsOverLappingInterval(roomOne.lowerBounds.x, roomOne.upperBounds.x, roomTwo.lowerBounds.x, roomTwo.upperBounds.x);
        bool isOverlappingY = IsOverLappingInterval(roomOne.lowerBounds.y, roomOne.upperBounds.y, roomTwo.lowerBounds.y, roomTwo.upperBounds.y);
    ...
    /// <summary>
    /// check if interval 1 overlaps with inverval 2 - this method is used by the IsOverLappingRoom method
    /// </summary>
    /// <param name="iminOne"></param>
    /// <param name="imaxOne"></param>
    /// <param name="iminTwo"></param>
    /// <param name="imaxTwo"></param>
    /// <returns></returns>
    private bool IsOverLappingInterval(int iminOne, int imaxOne, int iminTwo, int imaxTwo)
    {
        if(Mathf.Max(iminOne, iminTwo) <= Mathf.Min(iminOne, imaxOne))
        {
            return true;
        }
        else
    ...
    private RoomTemplateSO GetRandomRoomTemplate(RoomNodeTypeSO roomNodeType)
    {
        ...
        // Select reandom room template from list and return
    ...
    {
        // Iterate trhough all dungeon rooms
        foreach(KeyValuePair<string, Room> keyvaluepair in dungeonBuilderRoomDictionary)
        {
            Room room = keyvaluepair.Value;
